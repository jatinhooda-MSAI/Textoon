...
- **NumPy 2.x ABI break.** PyTorch install pulled NumPy 2.4 by default,
  incompatible with PyTorch's C extensions. Pinned to `numpy<2`.
- **libstdc++ version mismatch on HPC.** `optree` required GLIBCXX_3.4.31,
  newer than Quest's system libstdc++. Installed `libstdcxx-ng` from
  conda-forge and added an `activate.d` hook to prepend `$CONDA_PREFIX/lib`
  to `LD_LIBRARY_PATH`.
- **CUDA driver/toolkit mismatch.** PyTorch was built against CUDA newer than
  Quest's drivers supported. Reinstalled the CUDA 12.1 build of torch.
- **Sampling produced gray images despite healthy training loss.** Two bugs:
  `auto_normalize=True` in `GaussianDiffusion` double-normalized inputs already
  scaled to [-1, 1]; the hand-written CFG sampler had an incorrect posterior
  mean coefficient. Both fixed; samples then matched ground truth on the
  overfit test.
- **128×128 throughput shortfall.** Initial 128 run hit only 2 steps/sec on
  H100 — would not complete in budget. Diagnosis showed GPU-bound but with
  attention dominating cost. Resolved by enabling flash attention, dropping
  the U-Net's deepest stage from `dim_mults=(1,2,4,8)` to `(1,2,4,4)`,
  doubling batch size, and reducing target steps from 250k to 150k.
- **Attribute selection for evaluation.** Initial toggle experiments used
  attributes (cat_ears, witch_hat, wings) that the 64 model could not control.
  Frequency analysis on the training data revealed these were both rare
  (<3000 examples) and small in pixel footprint at 64×64. Reselected toggle
  attributes by jointly filtering for count >3000 and large pixel area.

## Limitations

- 48k images is light for pixel-space diffusion at 128×128. The model produces
  coherent samples but a larger dataset would likely improve fine detail and
  attribute disentanglement.
- Disentanglement is partial. Even at 128×128, attributes occupying very small
  pixel regions or rare in training data toggle weakly.
- Single multi-hot vocabulary (91 attributes) — no free-text conditioning.
